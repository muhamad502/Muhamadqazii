...
class PUBG_CHECKER:
		...
		while self.STR:
			...
			try:
				...
				if VV1ck.text.__contains__('"taken":true'):
					...
					post(f'https://api.telegram.org/bot{self.Telegrams[1]}/sendPhoto?chat_id={self.Telegrams[0]}&caption=👾 PUBG User :\n➖ [ {user} ] ➖\nBy @vv1ck',files={'photo': get('https://k.top4top.io/p_2814c924v0.jpeg').content})
	...
	def returns(self):
		if self.DV=='pc':print(f"\n{bloFT}┌──(xor㉿404)-[{wit}~{red}PUBG-MOBILE.exe{red}{bloFT}]\n└─${wit} The file name is incorrect !\n")
		else:print('\n┌──(xor㉿404)-[~PUBG-MOBILE.exe]\n└─$ The file name is incorrect !\n')
	def trts(self):
		if self.Modes=='1':
			self.Mod='File'
			try:
		...
		if self.DV=='pc':print(f"{bloFT}┌──(xor㉿404)-[{wit}~{red}PUBG-MOBILE.exe{red}{bloFT}]\n└─${wit} :")
		else:print('┌──(xor㉿404)-[~PUBG-MOBILE.exe]\n└─$ :')
...
if __name__ == '__main__':
	OS = input("┌──(xor㉿404)-[~PUBG-MOBILE.exe]\n└─$ What are you using ? \n\t\t[1] PC / rdp\n\t\t[2] mobile :")
	if OS == '1':DV='pc';os.system("cls" if os.name=='nt' else "clear");Modes = input(f"{bloFT}┌──(xor㉿404)-[{wit}~{red}PUBG-MOBILE.exe{red}{bloFT}]\n└─${wit} {yel}Modes{wit} :\n\t 1- Checking usernames from a file\n\t 2- Automatic checker\n->> Choose the mode: ")
	else:DV='me';os.system('clear');Modes = input(f"┌──(xor㉿404)-[~PUBG-MOBILE.exe]\n└─$Modes :\n\t 1- Checking usernames from a file\n\t 2- Automatic checker\n->> Choose the mode: ")
